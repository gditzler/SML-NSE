function [data_train,data_test,labels_train,labels_test] = test_then_train(data, labels, win_size, sample)
...
while true
  ...
  n = n + 1;
  if max(te_idx) > n_observations || isempty(te_idx)
  ...
  if sample
    i = randi(length(tr_idx), length(tr_idx), 1);
    j = randi(length(te_idx), length(te_idx), 1);
    ...
    data_train{n} = data_tr(i, :);
    data_test{n} = data_te(j, :);
    labels_train{n} = labels_tr(i);
    labels_test{n} = labels_te(j);
  else
